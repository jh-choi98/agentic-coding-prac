...
export default function App() {
  ...
  async doGenerate(
    ...
    const textParts = parts
      ...
      .map((p) => (p as { type: "text-delta"; id: string; delta: string }).delta)
      .join("");

    const toolCalls = parts
      .filter((p) => p.type === "tool-call")
      .map((p) => p as { type: "tool-call"; toolCallId: string; toolName: string; input: string });
    ...
    return {
      content,
      finishReason: (finishPart?.finishReason ?? "stop") as "stop" | "tool-calls",
      usage: {
        inputTokens: finishPart?.usage.inputTokens ?? 100,
        outputTokens: finishPart?.usage.outputTokens ?? 200,
        totalTokens: finishPart?.usage.totalTokens ?? 300,
      },
      warnings: [],
